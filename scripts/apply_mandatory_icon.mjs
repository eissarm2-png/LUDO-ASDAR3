...
const svgContent = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" width="1024" height="1024">
  <defs>
    ...
    <radialGradient id="bgGlow" cx="50%" cy="45%" r="62%">
      <stop offset="0%" stop-color="#64137c" />
      <stop offset="35%" stop-color="#3b074e" />
      <stop offset="70%" stop-color="#1b0325" />
      <stop offset="100%" stop-color="#0a010f" />
    </radialGradient>

    <!-- Outer Gold Border Gradient -->
    <linearGradient id="goldOuter" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="#FFF0A0" />
      <stop offset="22%" stop-color="#E5BA38" />
      <stop offset="48%" stop-color="#8C5C08" />
      <stop offset="76%" stop-color="#FDE882" />
      <stop offset="100%" stop-color="#704403" />
    </linearGradient>

    <!-- Inner Gold Border Gradient -->
    <linearGradient id="goldInner" x1="100%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%" stop-color="#FFF8B8" />
      <stop offset="30%" stop-color="#DFB136" />
      <stop offset="70%" stop-color="#784704" />
      <stop offset="100%" stop-color="#F5D266" />
    </linearGradient>

    <!-- Crown Gold Gradient -->
    <linearGradient id="crownGold" x1="0%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%" stop-color="#FFFFC8" />
      <stop offset="25%" stop-color="#FFD747" />
      <stop offset="65%" stop-color="#C78712" />
      <stop offset="100%" stop-color="#6E4002" />
    </linearGradient>

    <!-- Ruby Gemstone Gradient -->
    <radialGradient id="ruby" cx="35%" cy="35%" r="70%">
      <stop offset="0%" stop-color="#FF7A95" />
      <stop offset="35%" stop-color="#E6002E" />
      <stop offset="75%" stop-color="#7A0016" />
    ...
    <linearGradient id="textGold" x1="0%" y1="0%" x2="0%" y2="100%">
      ...
      <stop offset="15%" stop-color="#FFF4A8" />
      <stop offset="50%" stop-color="#EAA61E" />
      <stop offset="80%" stop-color="#915206" />
      <stop offset="100%" stop-color="#542F02" />
    </linearGradient>

    <!-- White Enamel Text (Al-Ludo) -->
    <linearGradient id="whiteEnamel" x1="0%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%" stop-color="#FFFFFF" />
      <stop offset="60%" stop-color="#F2F5FB" />
      <stop offset="100%" stop-color="#D4DCED" />
    </linearGradient>
  ...
  <g clip-path="url(#squircleInner)">
    ...
    <g opacity="0.5" transform="translate(0, 150)">
    ...
    <g filter="url(#shadowHeavy)" transform="translate(512, 230)">
      <path d="M -230,50 Q 0,85 230,50 Q 200,95 0,105 Q -200,95 -230,50 Z" fill="#6B3903" />
    ...
    <g font-family="system-ui, -apple-system, sans-serif" font-weight="900" font-size="168" text-anchor="middle">
    ...
    <g font-family="system-ui, -apple-system, sans-serif" font-weight="900" font-size="185" text-anchor="middle">
...
async function main() {
  console.log("Rendering 1024x1024 master icon from SVG using sharp...");
  const svgBuffer = Buffer.from(svgContent);

  const masterPng = await sharp(svgBuffer, { density: 300 })
    .resize(1024, 1024)
    .png({ quality: 100 })
    .toBuffer();

  const tempMasterPath = "/tmp/abqor_mandatory_icon.png";
  fs.writeFileSync(tempMasterPath, masterPng);
  console.log(`Saved master icon to ${tempMasterPath}`);

  // 1. Generate Web Assets
  const webSizes = [
    { file: "public/app-icon-512.png", size: 512 },
    { file: "public/app-icon-192.png", size: 192 },
    { file: "public/apple-touch-icon.png", size: 180 },
    { file: "public/favicon.png", size: 64 },
    { file: "public/favicon.ico", size: 32 },
  ];

  for (const { file, size } of webSizes) {
  ...
  // Also update brand-mark.png so the game header displays the new icon!
  const brandBuf = await sharp(masterPng).resize(256, 256).png().toBuffer();
  fs.writeFileSync("src/assets/brand-mark.png", brandBuf);
  if (fs.existsSync(".output/public/assets")) {
    fs.writeFileSync(".output/public/assets/brand-mark.png", brandBuf);
  }
  console.log("Updated src/assets/brand-mark.png");

  // 2. Generate Android Mipmaps
  const densities = {
    mdpi: { legacy: 48, adapt: 108 },
    hdpi: { legacy: 72, adapt: 162 },
    xhdpi: { legacy: 96, adapt: 216 },
    xxhdpi: { legacy: 144, adapt: 324 },
    xxxhdpi: { legacy: 192, adapt: 432 },
  };

  for (const [dens, { legacy, adapt }] of Object.entries(densities)) {
    const dir = `android/app/src/main/res/mipmap-${dens}`;
    fs.mkdirSync(dir, { recursive: true });

    // Legacy square
    const legBuf = await sharp(masterPng).resize(legacy, legacy).png().toBuffer();
    ...
    const roundBuf = await sharp(legBuf)
    ...
    const innerSize = Math.round(adapt * 0.72);
    const innerBuf = await sharp(masterPng).resize(innerSize, innerSize).png().toBuffer();
    ...
    console.log(`Updated Android mipmap-${dens}`);
  }

  // 3. Inject directly into APK
  const apkPath = "public/abqor-ludo.apk";
  if (fs.existsSync(apkPath)) {
    console.log("Injecting into public/abqor-ludo.apk...");

    // Generate WebP versions for APK injection
    const webpEntries = {};
    const densitiesV4 = {
      "mdpi-v4": { leg: 48, adapt: 108 },
      "hdpi-v4": { leg: 72, adapt: 162 },
      "xhdpi-v4": { leg: 96, adapt: 216 },
      "xxhdpi-v4": { leg: 144, adapt: 324 },
      "xxxhdpi-v4": { leg: 192, adapt: 432 },
    };

    for (const [dens, { leg, adapt }] of Object.entries(densitiesV4)) {
      // Legacy
      const lBuf = await sharp(masterPng).resize(leg, leg).webp({ lossless: true }).toBuffer();
      webpEntries[`res/mipmap-${dens}/ic_launcher.webp`] = lBuf;

      // Round
      const roundSvg = Buffer.from(
        `<svg width="${leg}" height="${leg}"><circle cx="${leg / 2}" cy="${leg / 2}" r="${leg / 2}" fill="#fff"/></svg>`,
      );
      const rBuf = await sharp(await sharp(masterPng).resize(leg, leg).png().toBuffer())
        .composite([{ input: roundSvg, blend: "dest-in" }])
        .webp({ lossless: true })
        .toBuffer();
      webpEntries[`res/mipmap-${dens}/ic_launcher_round.webp`] = rBuf;
      ...
      const fgBuf = await sharp({
        ...
        .webp({ lossless: true })
        .toBuffer();
      webpEntries[`res/mipmap-${dens}/ic_launcher_foreground.webp`] = fgBuf;

      // Background: Solid royal purple replacing the old green SWV background!
      const bgBuf = await sharp({
        create: {
          width: adapt,
          height: adapt,
          channels: 4,
          background: { r: 43, g: 6, b: 56, alpha: 1 },
        },
      })
        .webp({ lossless: true })
        .toBuffer();
      webpEntries[`res/mipmap-${dens}/ic_launcher_background.webp`] = bgBuf;
    }

    // Write WebP entries to a temp directory and use python zipfile to replace
    const tempDir = "/tmp/apk_icon_replacements";
    fs.rmSync(tempDir, { recursive: true, force: true });
    fs.mkdirSync(tempDir, { recursive: true });

    for (const [relPath, buffer] of Object.entries(webpEntries)) {
      const fullPath = path.join(tempDir, relPath);
      fs.mkdirSync(path.dirname(fullPath), { recursive: true });
      fs.writeFileSync(fullPath, buffer);
    }

    // Python script to repack zip safely preserving uncompressed resources.arsc
    const repackScript = `
import zipfile, os

apk_path = "${apkPath}"
temp_unsigned = "/tmp/abqor_unsigned.apk"
replacements_dir = "${tempDir}"

with zipfile.ZipFile(apk_path, "r") as zin, zipfile.ZipFile(temp_unsigned, "w") as zout:
    for item in zin.infolist():
        if item.filename.startswith("META-INF/"):
            continue
        rep_file = os.path.join(replacements_dir, item.filename)
        if os.path.exists(rep_file):
            with open(rep_file, "rb") as f:
                rep_data = f.read()
            zout.writestr(item.filename, rep_data, compress_type=zipfile.ZIP_DEFLATED)
        else:
            data = zin.read(item.filename)
            zout.writestr(item.filename, data, compress_type=item.compress_type)
print("Repacked unsigned APK.")
`;
    fs.writeFileSync("/tmp/repack.py", repackScript);
    execSync("python3 /tmp/repack.py", { stdio: "inherit" });

    // Zipalign 4
    console.log("Aligning APK with zipalign...");
    const tempAligned = "/tmp/abqor_aligned.apk";
    execSync(`zipalign -f -p 4 /tmp/abqor_unsigned.apk ${tempAligned}`, { stdio: "inherit" });

    // Resign with abqor-release.keystore
    console.log("Signing APK with apksigner...");
    execSync(
      `apksigner sign --ks abqor-release.keystore --ks-pass pass:abqor_ludo_2026 --key-pass pass:abqor_ludo_2026 --ks-key-alias abqor --min-sdk-version 21 --v1-signing-enabled true --v2-signing-enabled true --v3-signing-enabled true --out ${apkPath} ${tempAligned}`,
      { stdio: "inherit" },
    );

    // Sync copies
    if (fs.existsSync("public/ludo.apk")) fs.copyFileSync(apkPath, "public/ludo.apk");
    if (fs.existsSync(".output/public/abqor-ludo.apk"))
      fs.copyFileSync(apkPath, ".output/public/abqor-ludo.apk");
    if (fs.existsSync(".output/public/ludo.apk"))
      fs.copyFileSync(apkPath, ".output/public/ludo.apk");

    console.log("APK successfully updated with the mandatory icon and re-signed!");
  }

  console.log("Mandatory icon applied everywhere successfully!");
}

main().catch((err) => {
  console.error("Error applying mandatory icon:", err);
  process.exit(1);
});
