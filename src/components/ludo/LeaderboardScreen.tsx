...
export function Leaderboard({ meId }: { meId?: string | null }) {
  ...
  if (loading && !rows)
    return <p className="py-10 text-center text-ludo-soft">جارٍ تحميل المتصدرين…</p>;
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between px-1">
        ...
        <Button variant="ghostGold" size="sm" onClick={() => void load()}>
        ...
        <div key={row.id} className={cn("list-card", row.id === meId && "ring-2 ring-ludo-gold")}>
          ...
          <span className="min-w-0 flex-1">
            ...
            <small className="text-ludo-soft">
              {row.wins} فوز · آخر تحديث {formatDate(row.updated_at)}
            </small>
          </span>
