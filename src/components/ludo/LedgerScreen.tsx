  ...
  if (!entries.length) {
    return (
      <div className="glossy-card text-center">
        ...
        />
        <p className="relative mt-2 text-sm text-ludo-soft">
          لا توجد معاملات بعد — افتح صندوقًا أو أكمل مهمة.
        </p>
      </div>
    );
  }
      ...
      {entries.map((e) => (
        <article key={e.id} className="ledger-row">
          ...
          <div className="min-w-0 flex-1">
            ...
            <small className="block font-mono text-[10px] text-ludo-soft/70">
              #{e.id.slice(0, 8)}
            </small>
          </div>
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-1">
            {e.gold_delta !== 0 && (
              <Amount art={ART.gold} value={e.gold_delta} tone="var(--ludo-gold)" />
            )}
            {e.diamonds_delta !== 0 && (
