...
export function MatchSummary({ winnerName, events, onRestart, onHome }: Props) {
  ...
  return (
    <div
      className="fixed inset-0 z-[70] grid place-items-center bg-ludo-deep/85 p-4 backdrop-blur-sm"
      dir="rtl"
    >
      <div className="royal-panel celebrate-pop max-h-[92vh] w-full max-w-sm overflow-y-auto p-5 text-center">
        ...
        {last && (
          <section className="mb-3 rounded-xl border border-ludo-gold/35 bg-ludo-panel/70 p-3 text-right">
            ...
            <p className="text-xs text-ludo-soft">
              ...
              <b className="text-ludo-pink">{pathLabel(last.from)}</b> ←{" "}
              <b className="text-ludo-gold">{pathLabel(last.to)}</b>
            </p>
          </section>
        )}

        <Highlights
          title="أهم لقطات الالتقاط"
          icon={<Swords className="size-4" />}
          rows={captures}
          empty="لا التقاط في هذه المباراة"
        />
        <Highlights
          title="الوصول للمنزل"
        ...
        <Button variant="play" size="xl" className="mt-4 w-full" onClick={onRestart}>
          لعبة جديدة
        </Button>
