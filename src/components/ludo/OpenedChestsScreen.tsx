...
export function OpenedChestsPanel({ signedIn }: { signedIn: boolean }) {
  ...
  return (
    <div className="space-y-4">
          ...
          {openings.map((o) => (
            <article key={o.id} className="ledger-row">
              ...
              <div className="min-w-0 flex-1">
                ...
                <small className="block text-[11px] text-ludo-soft">
                  {arabicTime(o.created_at)}
                </small>
              </div>
              <div className="flex shrink-0 flex-wrap items-center justify-end gap-1">
                {o.gold_delta !== 0 && (
                  <Amount art={ART.gold} value={o.gold_delta} tone="var(--ludo-gold)" />
                )}
                {o.diamonds_delta !== 0 && (
                  <Amount art={ART.diamonds} value={o.diamonds_delta} tone="var(--ludo-lagoon)" />
                )}
                {o.xp_delta !== 0 && (
                  <Amount art={ART.xp} value={o.xp_delta} tone="var(--ludo-pink)" />
                )}
              </div>
            </article>
          ))}
      ...
      {items.length > 0 && (
        <section>
          <h4 className="mb-2 text-center text-sm font-bold text-ludo-gold">
            مقتنياتك من الصناديق
          </h4>
          <div className="grid grid-cols-3 gap-2">
            {items.map((it) => (
              ...
              >
                ...
                <small className="relative text-[10px] text-ludo-soft">
                  {ITEM_KIND_LABEL[it.kind]}
                </small>
                <span className="relative">
                  <RarityChip rarity={it.rarity} />
                </span>
                <small className="relative text-[9px] text-ludo-soft/70">
                  {arabicTime(it.created_at)}
                </small>
              </article>
            ))}
          </div>
