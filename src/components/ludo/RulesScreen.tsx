...
const STEPS: [string, string, React.ReactNode][] = [
  ...
  [
    "اخرج بالرقم 6",
    "لا تخرج القطعة من حوشها إلا عند ظهور الرقم 6، ثم تحصل على رمية إضافية.",
    <Flag key="2" />,
  ],
  [
    "تحرّك في المسار",
    "تسير القطعة بعدد نقاط النرد على المسار الدائري باتجاه ممرها المنزلي.",
    <Timer key="3" />,
  ],
  [
    "أطح بالخصوم",
    "إذا حللت في خانة خصم غير آمنة، تعود قطعته إلى الحوش وتحصل على رمية إضافية.",
    <Swords key="4" />,
  ],
  [
    "الخانات الآمنة",
    "الخانات المميزة بنجمة آمنة، لا يمكن الإطاحة بأي قطعة تقف عليها.",
    <Home key="5" />,
  ],
  [
    "الوصول للمنزل",
    "أوصل قطعتك بعدد دقيق إلى قلب اللوحة. أول من يُدخل قطعه الأربع يفوز.",
    <Crown key="6" />,
  ],
];
        ...
        {STEPS.map(([title, body, icon], i) => (
          <li key={title} className="list-card items-start">
            ...
            <span className="min-w-0 flex-1">
              <b className="flex items-center gap-2 text-ludo-gold">
                {icon}
                {title}
              </b>
