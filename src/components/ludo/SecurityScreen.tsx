  ...
  if (!user) {
    return (
      <p className="glossy-card text-center text-sm text-ludo-soft">
        سجّل الدخول لإدارة الحماية والأمان.
      </p>
    );
  }
  if (loading || !settings) {
  ...
  return (
    <div className="space-y-4">
      <section className="space-y-2">
        ...
        <Row label="تأكيد البريد" hint={user.email_confirmed_at ? "مؤكَّد" : "غير مؤكَّد"}>
          <ShieldCheck
            className={
              user.email_confirmed_at ? "size-5 text-ludo-palm" : "size-5 text-ludo-soft/60"
            }
          />
        </Row>
        <Button
      ...
      <section className="space-y-2">
        ...
        <Row label="من يمكنه رؤية ملفك">
          <select
            ...
            onChange={(e) =>
              void patch({
                profile_visibility: e.target.value as PlayerSettings["profile_visibility"],
              })
            }
        ...
        <Row label="السماح بدعوات اللعب" hint="استقبال دعوات الغرف من اللاعبين">
          <Switch
            ...
            onCheckedChange={(v) => void patch({ allow_invites: v })}
          />
        </Row>
        <Row label="إظهار حالة الاتصال" hint="ظهورك كمتصل للأصدقاء">
          <Switch
            checked={settings.show_online}
            onCheckedChange={(v) => void patch({ show_online: v })}
          />
        </Row>
      </section>
      ...
      <section className="space-y-2">
        ...
        {events.length === 0 ? (
          <p className="glossy-card py-4 text-center text-xs text-ludo-soft">
