  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const fresh = payload.new as Partial<Profile>;
          setProfile((prev) => (prev ? { ...prev, ...fresh } : (payload.new as Profile)));
        },
      )
      .subscribe();

    const onBalanceUpdated = (e: Event) => {
      const custom = e as CustomEvent<Partial<Profile>>;
      if (custom.detail) {
        setProfile((prev) => (prev ? { ...prev, ...custom.detail } : null));
      }
    ...
    return () => {
      ...
      window.removeEventListener("balance_updated", onBalanceUpdated);
    };
  }, [user, loadProfile]);

  const updateProfileLocally = useCallback((patch: Partial<Profile>) => {
