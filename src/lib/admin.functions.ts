...
export const adminListUsers = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: { search?: string; limit?: number; offset?: number }) => ({
...
export const adminTransferPoints = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator(
    (input: {
      userId: string;
      ...
      points: int(input?.points ?? 0, -1_000_000, 1_000_000),
      xp: int(input?.xp ?? 0, -1_000_000, 1_000_000),
      note: text(input?.note ?? "", 120),
    }),
  )
  .handler(async ({ data, context }): Promise<TransferPointsResult> => {
    const isSelf = context.userId === data.userId;

    // 1. Try atomic RPC admin_transfer_points
    let rpcRes: {
      ok?: boolean;
    ...
    try {
      ...
      )("admin_transfer_points", {
        _receiver_id: data.userId,
        _gold: data.gold,
        _diamonds: data.diamonds,
        _xp: data.xp,
        _points: data.points,
        _note: data.note || "تحويل إداري",
      });
      if (
        !error &&
        res &&
        typeof res === "object" &&
        "ok" in res &&
        Boolean((res as { ok: boolean }).ok)
      ) {
        rpcRes = res as typeof rpcRes;
      }
    } catch {
      // Fallback below
    }

    // 2. Fallback to admin_adjust_economy if admin_transfer_points is not present
    if (!rpcRes) {
      const { error: adjustErr } = await context.supabase.rpc("admin_adjust_economy", {
        _uid: data.userId,
        _gold: data.gold,
        _diamonds: data.diamonds,
        _xp: data.xp,
      ...
      if (adjustErr) {
        return { ok: false, reason: adjustErr.message || "forbidden" };
      }
    }
    ...
    const { data: profileData } = await context.supabase
      .from("profiles")
      .select("id, gold, diamonds, points, xp, level")
      .eq("id", data.userId)
      .maybeSingle();

    // 4. Build notification content
    const parts: string[] = [];
    if (data.gold > 0) parts.push(`+${data.gold} ذهب`);
    if (data.diamonds > 0) parts.push(`+${data.diamonds} ألماس`);
    if (data.points > 0) parts.push(`+${data.points} نقطة`);
    if (data.xp > 0) parts.push(`+${data.xp} XP`);

    const title = "تم استلام نقاط جديدة 🎉";
    const body =
      parts.length > 0
        ? `تم إضافة ${parts.join(" و ")} إلى رصيدك${data.note ? ` (${data.note})` : ""}`
        : `تم تحديث رصيدك من قبل الإدارة${data.note ? ` (${data.note})` : ""}`;

    // 5. Create in-app notification row if not already created
    let notifId = rpcRes?.notification_id;
    if (!notifId) {
      try {
        const { data: nData } = await context.supabase
          .from("notifications")
          .insert({
            user_id: data.userId,
            kind: "points_transfer",
            title,
            body,
          })
          .select("id")
          .maybeSingle();
        if (nData?.id) notifId = nData.id;
      } catch {
        // Notification insertion may be subject to RLS or already handled by RPC
      }
    }
    ...
    return {
      ...
      reason: "ok",
      isSelf,
      profile: (profileData as TransferPointsResult["profile"]) ?? rpcRes?.profile,
      notification: {
        id: notifId,
        title,
        body,
...
export const adminAdjustEconomy = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator(
    (input: { userId: string; gold?: number; diamonds?: number; xp?: number; note?: string }) => ({
      userId: uuid(input?.userId),
      gold: int(input?.gold ?? 0, -1_000_000, 1_000_000),
      diamonds: int(input?.diamonds ?? 0, -100_000, 100_000),
      xp: int(input?.xp ?? 0, -1_000_000, 1_000_000),
...
export const adminSetBan = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: { userId: string; banned: boolean; reason?: string }) => ({
    userId: uuid(input?.userId),
...
export const adminSetRole = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: { userId: string; role: "admin" | "moderator" | "user"; grant: boolean }) => {
...
export const adminGrantItem = createServerFn({ method: "POST" })
  ...
  .validator(
    (input: {
      ...
      if (kind !== "avatar" && kind !== "banner" && kind !== "frame")
        throw new Error("invalid kind");
      return {
        userId: uuid(input?.userId),
        kind,
        code: text(input?.code, 40),
        rarity: text(input?.rarity ?? "rare", 20),
      };
    },
  )
  .handler(async ({ data, context }) => {
...
export const adminRecentMatches = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: { limit?: number }) => ({ limit: int(input?.limit ?? 40, 1, 200) }))
  .handler(async ({ data, context }) => {
    const { data: rows, error } = await context.supabase.rpc("admin_recent_matches", {
      _limit: data.limit,
    });
...
export const adminLogs = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: { limit?: number }) => ({ limit: int(input?.limit ?? 50, 1, 200) }))
  .handler(async ({ data, context }) => {
    const { data: rows, error } = await context.supabase.rpc("admin_logs_list", {
      _limit: data.limit,
    });
...
export const adminTurnEvents = createServerFn({ method: "POST" })
  ...
  .validator((input: { limit?: number }) => ({ limit: int(input?.limit ?? 50, 1, 200) }))
  .handler(async ({ data, context }) => {
    const { data: rows, error } = await context.supabase.rpc("admin_turn_events", {
      _limit: data.limit,
    });
...
export const adminToggleChest = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator(
    (input: { code: string; active: boolean; costGold?: number; costDiamonds?: number }) => ({
      code: text(input?.code, 40),
...
export const adminToggleMission = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: { code: string; active: boolean; goal?: number; rewardGold?: number }) => ({
    code: text(input?.code, 40),
    active: Boolean(input?.active),
