...
function nextTurn(state: GameState): GameState {
  ...
  const player = state.players[turn] ??
    state.players[0] ?? { seat: 0 as SeatId, name: "لاعب 1", isBot: false };
  return {
    ...state,
    turn,
    phase: "roll",
...
export function pickBotMove(moves: Move[]): Move {
  const score = (m: Move) =>
    (m.captures.length ? 1000 : 0) + (m.finishes ? 800 : 0) + (m.entersBoard ? 400 : 0) + m.to;
  const selected = [...moves].sort((a, b) => score(b) - score(a))[0];
  return (
    selected ?? { tokenId: "", from: -1, to: -1, captures: [], finishes: false, entersBoard: false }
  );
}
