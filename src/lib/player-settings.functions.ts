...
export const savePlayerSettings = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((data: Partial<PlayerSettings>) => data)
...
export const listSecurityEvents = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<SecurityEvent[]> => {
    const { data, error } = await context.supabase.rpc("list_security_events", {
      _limit: 50,
    } as never);
    if (error) throw new Error(error.message);
    const rows =
      (data as unknown as { id: string; action: string; detail: unknown; created_at: string }[]) ??
      [];
    return rows.map((r) => ({
      id: r.id,
      action: r.action,
...
export const logSecurityEvent = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((data: { action: string; detail?: string }) => data)
