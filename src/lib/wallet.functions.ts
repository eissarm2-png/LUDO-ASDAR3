...
export const chargeDiceRoll = createServerFn({ method: "POST" })
  ...
  .validator((input: { cost?: number; matchId?: string }) => {
    const cost = Math.round(Number(input?.cost ?? 2));
    if (!Number.isFinite(cost) || cost < 0 || cost > 50) throw new Error("invalid cost");
    const matchId =
      typeof input?.matchId === "string" && UUID_RE.test(input.matchId) ? input.matchId : null;
    return { cost, matchId };
  })
  .handler(async ({ data, context }) => {
    ...
    if (error)
      return { ok: false, reason: error.message, gold: 0, diamonds: 0 } satisfies RollCharge;
